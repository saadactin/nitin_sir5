CREATE PROCEDURE OCSV_STAGING_DELTA
(
)
AS
BEGIN
		DELETE FROM ETL_SALES_LINES WHERE DOCTYPE = 166 AND DOCENTRY IN (SELECT "DocEntry" FROM OCSV_STAGING_DELTA_TABLE);
		INSERT INTO ETL_SALES_LINES
			...
			INNER JOIN "OCSV_STAGING_DELTA_TABLE" TEMP ON TEMP."DocEntry" = T0."DocEntry"
			Left Outer Join 
				(
