CREATE PROCEDURE OCR1_COST_CENTER_DELTA()
AS
COUNTER  Integer := 0;
BEGIN
	DELETE FROM "ETL_COST_CENTER" T1 WHERE EXISTS (SELECT 1 FROM "OCR1_COST_CENTER_DELTA_TABLE" TT WHERE T1."OcrCode"=TT."OcrCode" AND T1."PrcCode"=TT."PrcCode");
	INSERT INTO "ETL_COST_CENTER" 
		...
		FROM (
			...
			INNER JOIN "OCR1_COST_CENTER_DELTA_TABLE" TT ON T0."OcrCode"=TT."OcrCode" AND T0."PrcCode"=TT."PrcCode" AND T0."ValidFrom" = TT."ValidFrom" AND TT.OPT IN ('I','U')) T2
		INNER JOIN "OPRC" T3 ON T2."PrcCode"=T3."PrcCode"
		INNER JOIN "ODIM" T4 ON T2."DimCode"=T4."DimCode"
		WHERE T4."DimActive"='Y';
		
	--SELECT COUNT(*) INTO COUNTER FROM "ETL_BUDGET_COSTCENTER" T1, "OCR1_COST_CENTER_DELTA_TABLE" TT WHERE T1."CODE" = TT."PrcCode" AND TT.OPT='I';
	--IF COUNTER = 0 THEN
		DELETE FROM "ETL_BUDGET_COSTCENTER" WHERE "CODE" IN (SELECT DISTINCT "PrcCode" FROM OCR1_COST_CENTER_DELTA_TABLE);
		...
		INNER JOIN (SELECT DISTINCT "PrcCode" FROM "OCR1_COST_CENTER_DELTA_TABLE" WHERE OPT IN ('I', 'U')) TT ON T0."PrcCode" = TT."PrcCode";
	--END IF;
END
