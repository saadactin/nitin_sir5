CREATE PROCEDURE ODIM_COST_CENTER_DELTA()
AS
BEGIN
		DELETE FROM "ETL_COST_CENTER" WHERE "DimCode" IN (SELECT "DimCode" FROM ODIM_COST_CENTER_DELTA_TABLE) ;
		INSERT INTO "ETL_COST_CENTER"
			...
			INNER JOIN ODIM_COST_CENTER_DELTA_TABLE TEMP ON TEMP."DimCode" = T4."DimCode"
			WHERE T4."DimActive"='Y';
		
		
			DELETE FROM "ETL_BUDGET_COSTCENTER" WHERE "DIM_CODE" IN (SELECT "DimCode" FROM ODIM_COST_CENTER_DELTA_TABLE) ;
			...
			FROM DUMMY CROSS JOIN  ODIM T0 
			INNER JOIN ODIM_COST_CENTER_DELTA_TABLE TEMP
			ON T0."DimCode" = TEMP."DimCode" 
			WHERE T0."DimActive" = 'Y';
			...
			INNER JOIN ODIM_COST_CENTER_DELTA_TABLE TEMP ON TEMP."DimCode" = T1."DimCode"
