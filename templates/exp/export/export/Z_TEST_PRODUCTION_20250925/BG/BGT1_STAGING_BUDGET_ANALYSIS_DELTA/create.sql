CREATE PROCEDURE BGT1_STAGING_BUDGET_ANALYSIS_DELTA 
() 
LANGUAGE SQLSCRIPT AS
Account NVARCHAR(15) := '';
Num	INTEGER;
BEGIN
	SELECT COUNT(*) INTO Num FROM BGT1_STAGING_BUDGET_ANALYSIS_DELTA_TABLE WHERE "OPT" = 'D';
	IF :Num > 0 THEN
		DELETE FROM "ETL_BUDGET_ANALYSIS" WHERE NOT EXISTS(
						SELECT 1 FROM BGT1 T0 INNER JOIN OBGT T1 ON T1."AbsId" = T0."BudgId" AND T0."AcctCode" = T1."AcctCode" 
						WHERE "BUDGET_SCENARIO_ID" = T1."Instance" AND "BUDGET_DAY" = ADD_MONTHS(T1."FinancYear", T0."Line_ID") AND 
		                    "ACCOUNT_CODE" = T1."AcctCode"
		);
	END IF;
	
	SELECT COUNT(*) INTO Num FROM "OBGT" T0
	INNER JOIN BGT1_STAGING_BUDGET_ANALYSIS_DELTA_TABLE TEMP
	ON T0."AbsId" = TEMP."BudgId";
	
	IF :Num = 0 THEN
		RETURN;
	END IF;
	
	ACCOUNT_INFO = 	SELECT DISTINCT T0."AcctCode" FROM "OBGT" T0
					INNER JOIN BGT1_STAGING_BUDGET_ANALYSIS_DELTA_TABLE TEMP
					ON T0."AbsId" = TEMP."BudgId";
