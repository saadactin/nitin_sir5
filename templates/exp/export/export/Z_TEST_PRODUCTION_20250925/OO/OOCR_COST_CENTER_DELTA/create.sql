CREATE PROCEDURE OOCR_COST_CENTER_DELTA()
AS
BEGIN
	DELETE FROM "ETL_COST_CENTER" T1 WHERE EXISTS (SELECT 1 FROM "OOCR_COST_CENTER_DELTA_TABLE" TT WHERE T1."OcrCode"=TT."OcrCode" AND TT.OPT='U');
	INSERT INTO "ETL_COST_CENTER"
		...
		FROM (
			...
			FROM "OCR1" T0
			INNER JOIN "OOCR" T1 ON T0."OcrCode"=T1."OcrCode") T2
		INNER JOIN "OPRC" T3 ON T2."PrcCode"=T3."PrcCode"
		INNER JOIN "ODIM" T4 ON T2."DimCode"=T4."DimCode"
		INNER JOIN "OOCR_COST_CENTER_DELTA_TABLE" TT ON T2."OcrCode"=TT."OcrCode" AND TT.OPT='U'
