...
# Credentials will be loaded from Jarvis_cred database
# These are placeholders - actual values loaded from database at runtime
PG_HOST = None
PG_PORT = None
PG_DATABASE = None
PG_USERNAME = None
...
CLICKHOUSE_PASS = None
CLICKHOUSE_DB = None

def load_credentials_from_db(pg_host, pg_port, pg_username, pg_password):
    """
    Load credentials from Jarvis_cred database
    This function is called before migration starts
    """
    import sys
    import os
    ...
    from migrations.credential_manager import get_all_migration_credentials
    
    global PG_HOST, PG_PORT, PG_DATABASE, PG_USERNAME, PG_PASSWORD
    global CLICKHOUSE_HOST, CLICKHOUSE_USER, CLICKHOUSE_PASS, CLICKHOUSE_DB
    
    try:
        pg_creds, ch_creds = get_all_migration_credentials(pg_host, pg_port, pg_username, pg_password)
        
        if pg_creds:
            PG_HOST = pg_creds.get('pg_host')
            PG_PORT = pg_creds.get('pg_port', 5432)
            PG_DATABASE = pg_creds.get('pg_database')
            PG_USERNAME = pg_creds.get('pg_username')
            PG_PASSWORD = pg_creds.get('pg_password')
        
        if ch_creds:
            CLICKHOUSE_HOST = ch_creds.get('ch_host')
            CLICKHOUSE_USER = ch_creds.get('ch_user')
            CLICKHOUSE_PASS = ch_creds.get('ch_password')
            CLICKHOUSE_DB = ch_creds.get('ch_database')
        
        logger.info("Credentials loaded from Jarvis_cred database")
        return True
...
def main(pg_host=None, pg_port=None, pg_username=None, pg_password=None):
    """
    Main migration function
    If pg_host is provided, load credentials from Jarvis_cred database
    Otherwise, use hardcoded values (for backward compatibility)
    """
    logger.info("Starting PostgreSQL to ClickHouse migration")
    
    # Load credentials from database if provided
    if pg_host:
        logger.info("Loading credentials from Jarvis_cred database...")
        load_credentials_from_db(pg_host, pg_port or 5432, pg_username, pg_password)
    
    # Validate credentials are loaded
    if not PG_HOST or not PG_DATABASE or not PG_USERNAME or not PG_PASSWORD:
        logger.error("PostgreSQL credentials not loaded. Please ensure credentials are saved in Jarvis_cred database.")
        return
    
    if not CLICKHOUSE_HOST or not CLICKHOUSE_DB or not CLICKHOUSE_USER or not CLICKHOUSE_PASS:
        logger.error("ClickHouse credentials not loaded. Please ensure credentials are saved in Jarvis_cred database.")
        return
    ...
    try:
        ...
        if len(tables) == 0:
            ...
            return {'tables_migrated': 0, 'tables': [], 'status': 'completed'}
        ...
        logger.info(f"Successfully migrated {len(migrated_tables)} tables")
        logger.info("="*60)
